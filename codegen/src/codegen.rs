...
impl<'a, 'c> CodeGen<'a, 'c> {
    ...
    fn resolve_any_ident(&mut self, ident: &IdentKey, expr: Option<&Expr>) -> VirtualAddr {
        ...
        match mode {
            ...
            OperandMode::Value => {
                let id = &self.ast_objs.idents[*ident];
                match &*id.name {
                    "true" => func_ctx!(self).add_const(GosValue::new_bool(true)),
                    "false" => func_ctx!(self).add_const(GosValue::new_bool(false)),
    ...
    fn add_local_or_resolve_ident(
        ...
        if is_def {
            ...
            if ctx.is_ctor(&self.objects.functions) {
                ...
                pkg.add_var_mapping(ident.name.clone(), index.as_direct_addr().as_var_index());
            }
    ...
    fn add_local_var(&mut self, okey: TCObjKey) -> (VirtualAddr, TCTypeKey, Meta) {
    ...
    fn gen_const(&mut self, node: NodeId, pos: Option<Pos>) {
        ...
        match ectx.mode {
            ExprMode::Load => ectx.load_addr = addr.as_direct_addr(),
            ExprMode::Assign(d) => fctx.emit_assign(d, addr.as_direct_addr(), pos),
        }
    }
    ...
    fn visit_expr(&mut self, expr: &Expr, cur_reg: OpIndex, mode: ExprMode) {
        self.push_expr_ctx(cur_reg, mode);
    ...
    fn push_expr_ctx(&mut self, cur_reg: OpIndex, mode: ExprMode) {
        self.expr_ctx_stack.push(ExprCtx::new(cur_reg, mode));
    }

    fn pop_expr_ctx(&mut self) {
        let ctx = self.expr_ctx_stack.pop().unwrap();
        func_ctx!(self).update_max_reg(ctx.max_reg);
    }
...
impl<'a, 'c> ExprVisitor for CodeGen<'a, 'c> {
    ...
    fn visit_expr_ident(&mut self, expr: &Expr, ident: &IdentKey) {
        let addr = self.resolve_any_ident(ident, Some(expr));
        ...
        match ectx.mode {
            ExprMode::Load => ectx.load_addr = addr.as_direct_addr(),
            ExprMode::Assign(sa) => fctx.emit_assign(sa, addr.as_direct_addr(), p),
        }
    }
    ...
    fn visit_expr_func_lit(&mut self, this: &Expr, flit: &FuncLit) {
        ...
        let mut fctx = func_ctx!(self);
        let addr = fctx
            .add_const(GosValue::new_function(fkey))
            .as_direct_addr();
        let pos = Some(flit.body.l_brace);
        let ectx = expr_ctx!(self);
        let (dest, assign_dest) = ectx.get_dest();
        fctx.emit_closure(dest, addr, pos);
        if let Some(va) = assign_dest {
            fctx.emit_assign(*va, dest, pos);
        }
    }
